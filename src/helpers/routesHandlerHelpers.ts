  ...
  try {
    ...
    if (path === '/api/users' && req.method === httpMethods.GET) {
      ...
      req.on('end', async () => {
        ...
        const updatedUser = await DBStorage.updateUser(id, updateUserData).then(
          (user: IUser | undefined | null) => user
        );

        return updatedUser
          ? getResponse(res, 200, updatedUser)
          : updatedUser === null
            ? getResponse(res, 400, MESSGES.ERROR_400)
            : getResponse(res, 404, `${MESSGES.ERROR_NOT_FOUND} ${id}`);
      });
    } else if (path.startsWith('/api/users/') && req.method === httpMethods.DELETE) {
      const id: string = path.substring('/api/users/'.length);
      ...
      const users = await DBStorage.deleteUser(id).then((users: IUser[] | undefined) => users);

      return users
        ? getResponse(res, 200, `${MESSGES.USER_DELETED} ${id}`)
        : getResponse(res, 404, `${MESSGES.ERROR_NOT_FOUND} ${id}`);
    } else {
